...
It also handles brand-new files better now by inferring likely related files and tests even before they have indexed history, adds lightweight Go, JS/TS, and Python dependency signals including TS path aliases, prefers real repo verification commands when they exist, and surfaces boundary warnings for areas like auth, config, DB, API, payments, and jobs.
